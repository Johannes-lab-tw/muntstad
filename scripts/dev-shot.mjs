...
if (screens.includes('avontuur')) { await page.locator('#nav-avontuur').click(); await page.waitForTimeout(1200); await shot('10-avontuur'); await page.evaluate(() => window.__muntstad.avontuur.setPhase(0.8)); await page.waitForTimeout(700); await shot('11-avontuur-nacht'); await page.evaluate(() => window.__muntstad.avontuur.setPhase(null)); await page.waitForTimeout(700); await closePopups(); await page.locator('#av-dorp').click(); await page.locator('#screen-dorp.active').waitFor({ timeout: 20000 }); await page.waitForTimeout(600); await closePopups(); await page.locator('#dp-kaart').click(); for (let i = 0; i < 10 && !(await page.locator('#nav-werk').isVisible()); i++) { await page.waitForTimeout(500); await closePopups(); } }   // since V6.3 DORP is the walkable village; KAART (#dp-kaart) goes back to the town map with the WERK button
