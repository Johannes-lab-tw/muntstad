...
test('KAMP sells the backpack for coins and the axe is bought with the shared wallet', async ({ page }) => {
  const errors = watchErrors(page);
  await seedSave(page, (s) => { s.wallet = 50; s.earnedWork = 50; s.eiland = { bag: { hout: 5, schelp: 4, bes: 0, vis: 1 }, tools: {}, quest: 0, questN: 0, questsDone: 0, collected: {}, sold: 0, earned: 0 }; return s; });
  await startGame(page);
  await closePopups(page);
  await openAvontuur(page);
