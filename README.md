...
- **AVONTUUR** (PLAN-V4, September 2026) — the boat to the Avontuureiland: walk around in third person (joystick, swipe, SPRING), chop wood, pick shells and berries, fish, sell at the campfire, buy the axe/lantern/fence/tent/torches, Muntje's daily quests; a 13-minute day/night cycle (8 + 5, since PLAN-V6) with ghosts that steal in the dark, shadow wolves from night 5, the Nachtbeer every third night and a reward at dawn; SAMEN SPELEN with up to six players through a tiny relay on Cloudflare Workers (server/relay; room code = four pictures, host runs the world).
...
- **PLAN-V6** (5 September 2026, "one world, real survival") — the island five times bigger in 60 m tiles, fire levels by wood (KOOK), cold, a night counter, shadow wolves from night 5, quest chains, SAMEN on the island and WEK, a minimap, a walkable town (DORP) with a harbour and the boat (VAAR), ten maker levels, Hotel/Handelshaven/Raketbasis, show pieces and the Spaarbank (5 % a day).
- **V7.1 – V7.4** (13–14 September 2026, PLAN-V7 "kind-eerst") — the STOOK bug at a nearly full fire (the last fraction of room now takes a whole piece; "Het vuur is vol" instead of "geen hout"; guests see the host's fire up to 400); a picture-first UI kit (`.btn-ico` round picture buttons in four sizes, `.ring` meters, `.tile` badges) on the island and the town screens, six "buttons out of the way" rules guarded by an e2e audit (free middle, one big action, dimming while walking, emotes behind one smiley); and the fire as the hero (bigger flame per level, a low ring instead of a tipi, a five-log level meter above the flame, picture signposts, a lantern, a wood pile). Concepts came from Higgsfield with the iPad photo as reference; they are a moodboard, never assets.
- **V7.0** (6 September 2026, after the third iPad test) — rotating the iPad no longer leaves the island blue (a window resize used to pull the shared canvas into the hidden town screen), and the island HUD folds the campaign and chain cards to one line each (tap to unfold; a finished step unfolds by itself).
- **PLAN-V5** (September 2026, from the six-year-old's own feedback) — trees that fall, a bent cave with a chamber, bats and a cave ghost, hunger and EET, fainting, the Nachthert, nights that get harder, seven gadgets and camp upgrades, a wash bay that grows with its level, a bigger town with IJssalon and Pretpark, a livelier voice and two music themes.
...
docs/               the deployed site (open with any static server)
  ...
  js/samen-wereld.js  the shared world of SAMEN SPELEN (pure): the host's world message, a guest's shot checked, nearest player
