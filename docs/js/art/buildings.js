...
// Every builder: (iso, ctx, x, y, level, t, opts) with (x, y) the centre of a 3 × 3 plot, level 1..5, t in ms.
// opts.pave = false skips the pavement (the town draws pavements in its static layer, under the road).
// opts.part = 'static' draws only the parts that never move (the town caches those as a sprite),
// opts.part = 'anim' only the moving parts (smoke, drips, flags, floating coins); default: everything.
...
function parts(opts) {
  return { S: opts.part !== 'anim', A: opts.part !== 'static', pave: opts.pave !== false && opts.part !== 'anim' };
}
...
export function crown(iso, ctx, x, y, z, t, S = true, A = true) {
  if (S) {
...
// ---------- Limonadekraam ----------
export function limonade(iso, ctx, x, y, level, t, opts = {}) {
  const { S, A, pave } = parts(opts);
  const c = '#ffd23f';
  if (pave) pavement(iso, x, y);
  if (S) {
    ...
    if (level >= 5) crown(iso, ctx, x, y - 0.9, 2.45, 0, true, false);
  }
  if (A) {
    if (level >= 4) flag(iso, x - 1.4, y + 0.9, t, '#45d65c');
    if (level >= 5) crown(iso, ctx, x, y - 0.9, 2.45, t, false, true);
  }
}

// ---------- Wasstraat ----------
export function wasstraat(iso, ctx, x, y, level, t, opts = {}) {
  const { S, A, pave } = parts(opts);
  const c = '#4fb6ff';
  if (pave) pavement(iso, x, y);
  if (S) {
    ...
    if (level >= 3) iso.block(x - 1.2, y + 1.05, 0, 2.4, 0.4, 1, shade(c, 0.15)); // annex
    if (level >= 5) crown(iso, ctx, x, y - 1, 2.35, 0, true, false);
  }
  if (A) {
    ...
    if (level >= 5) crown(iso, ctx, x, y - 1, 2.35, t, false, true);
  }
}

// ---------- Pizzeria ----------
export function pizzeria(iso, ctx, x, y, level, t, opts = {}) {
  const { S, A, pave } = parts(opts);
  ...
  if (S) {
    ...
    if (level >= 5) crown(iso, ctx, x, y - 1, level >= 3 ? 3.4 : 2.5, 0, true, false);
  }
  if (A) {
    smoke(iso, ctx, x - 0.55, y - 0.45, level >= 3 ? 3.5 : 2.7, t, 900);
    if (level >= 4) flag(iso, x + 1.35, y + 1.3, t, '#ff5f5f');
    if (level >= 5) crown(iso, ctx, x, y - 1, level >= 3 ? 3.4 : 2.5, t, false, true);
  }
}

// ---------- Fabriek ----------
export function fabriek(iso, ctx, x, y, level, t, opts = {}) {
  const { S, A, pave } = parts(opts);
  const c = '#b794f4';
  if (pave) pavement(iso, x, y);
  if (S) {
    ...
    if (level >= 2) { iso.block(x - 1.45, y + 1.05, 0, 0.6, 0.4, 0.6, '#ff9f2e'); iso.block(x - 0.8, y + 1.05, 0, 0.6, 0.4, 0.45, '#45d65c'); } // crates
    if (level >= 3) { iso.block(x + 0.6, y + 1.05, 0, 0.9, 0.4, 1.0, shade(c, 0.1)); window(iso, x + 0.6, y + 1.05, 0, 0.9, 0.4, 'y', 0.2, 0.35, 0.5, 0.4, LIT); } // annex
    if (level >= 5) crown(iso, ctx, x - 0.2, y - 1.05, 3.1, 0, true, false);
  }
  if (A) {
    smoke(iso, ctx, x + 0.8, y - 0.4, 3.2, t, 1100, 10);
    if (level >= 4) flag(iso, x - 1.45, y - 1.4, t, '#b794f4', 2.9);
    if (level >= 5) crown(iso, ctx, x - 0.2, y - 1.05, 3.1, t, false, true);
  }
}

// ---------- Flatgebouw ----------
export function flat(iso, ctx, x, y, level, t, opts = {}) {
  const { S, A, pave } = parts(opts);
  const c = '#8fc7ff';
  ...
  if (S) {
    ...
    if (level >= 2) iso.block(x - 1.45, y + 1.05, 0, 0.9, 0.4, 0.5, '#45d65c'); // hedge
    ...
    if (level >= 5) { iso.block(x - 0.8, y + 0.1, h + 0.14, 0.9, 0.6, 0.08, '#6fd35b'); iso.tree(x - 0.4, y + 0.4, 0.45, undefined, undefined, h + 0.22); crown(iso, ctx, x, y - 1.05, h + 0.6, 0, true, false); } // roof garden
  }
  if (A) {
    if (level >= 4) flag(iso, x - 1.4, y - 1.4, t, '#ffd23f', h + 0.9);
    if (level >= 5) crown(iso, ctx, x, y - 1.05, h + 0.6, t, false, true);
  }
}

/** Height of a coin-maker in world units at a level (for sprites that must fit the whole building). */
...
/** The child's house with garden: paint is a fun item id (verf-*) or 'none'. opts: { patch, part } as above. */
export function house(iso, ctx, x, y, paint = 'none', t = 0, opts = {}) {
  const { S, A } = parts(opts);
  const wall = PAINT[paint] || PAINT.none;
  if (S) {
    if (opts.patch !== false) iso.groundRect(x - 1.7, y - 1.5, 3.4, 3, 0.5, '#bfe9a4', shade('#bfe9a4', -0.25));
  ...
  if (A) smoke(iso, ctx, x + 0.45, y - 0.35, 2.5, t, 1600, 6);
}
